...
- Theme styles such as color/font size/font style/rounded corners/shadow can be configured through json files. Get theme data through `TTheme.of(context)` or `TTheme.defaultData()`. It is recommended that all components use `TTheme.of(context)`Only components that do not need to follow the local theme can use `TTheme.defaultData()`.
